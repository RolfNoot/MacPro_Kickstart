...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPIS_HOST) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: SPIS_HOST_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
void SPIS_HOST_CyBtldrCommStart(void) CYSMALL 
{
    ...
    SPIS_HOST_Start();
    SPIS_HOST_ClearRxBuffer();
    SPIS_HOST_ClearTxBuffer();
}


/*******************************************************************************
* Function Name: SPIS_HOST_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIS_HOST_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIS_HOST_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    (void) SPIS_HOST_ReadTxStatus();

    /* Write TX data using blocking function */
    #if(0u != SPIS_HOST_MODE_USE_ZERO)
        SPIS_HOST_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    #endif /* (SPIS_HOST_MODE_USE_ZERO == 1u) */

    /* Write TX data using blocking function */
    do
    {
        SPIS_HOST_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (SPIS_HOST_ReadTxStatus() & SPIS_HOST_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
...
/*******************************************************************************
* Function Name: SPIS_HOST_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < (10u * (uint16) timeOut); iCntr++)
    {
        ...
        */
        if(0u != SPIS_HOST_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = SPIS_HOST_GetRxBufferSize();
                CyDelay(SPIS_HOST_BYTE2BYTE_TIME_OUT);
            }while(SPIS_HOST_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS;
            break;
        }
    ...
    *count = 0u;
    ...
    while(SPIS_HOST_GetRxBufferSize() > 0u)
    {
        tempCount = SPIS_HOST_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = SPIS_HOST_ReadRxData();
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPIS_HOST) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
